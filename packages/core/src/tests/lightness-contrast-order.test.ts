    ...
    describe.each([1, 2])("image %s", (n) => {
      ...
      it.each(["red", "blue"])(
        ...
        async (colorName: string) => {
          const reduced = await generateColorScheme(img, {
            mode,
            lightnessMultiplier: 0.5,
          });
          const normal = await generateColorScheme(img, {
            mode,
            lightnessMultiplier: 1,
          });
          const increased = await generateColorScheme(img, {
            mode,
            lightnessMultiplier: 1.5,
          });
      ...
      it.each(["red", "blue"])(
        ...
        async (colorName: string) => {
          const low = await generateColorScheme(img, {
            mode,
            contrastMultiplier: 0.5,
          });
          const mid = await generateColorScheme(img, {
            mode,
            contrastMultiplier: 1,
          });
          const high = await generateColorScheme(img, {
            mode,
          ...
          const samples = [
            {
              hex:
                gLow.values[idx]?.value ??
                gLow.values[gLow.values.length - 1].value,
              lbg: getLightnessValue(low.terminal[0]),
            },
            {
              hex:
                gMid.values[idx]?.value ??
                gMid.values[gMid.values.length - 1].value,
              lbg: getLightnessValue(mid.terminal[0]),
            },
            {
              hex:
                gHigh.values[idx]?.value ??
                gHigh.values[gHigh.values.length - 1].value,
              lbg: getLightnessValue(high.terminal[0]),
            },
          ];
          ...
          const sorted = [...samples].sort((a, b) => {
            ...
            if (Math.abs(diff) <= EPS)
              throw new Error("Tie in contrast deltas");
            return diff;
          });
          expect(sorted.map((s) => s.hex)).toEqual(
            expectedOrder.map((s) => s.hex),
          );
        },
      );
      ...
      it.each([9, 12])(
        ...
        async (termIndex: number) => {
          ...
          const samples = [
            {
              hex: low.terminal[termIndex],
              lbg: getLightnessValue(low.terminal[0]),
            },
            {
              hex: mid.terminal[termIndex],
              lbg: getLightnessValue(mid.terminal[0]),
            },
            {
              hex: high.terminal[termIndex],
              lbg: getLightnessValue(high.terminal[0]),
            },
          ];
